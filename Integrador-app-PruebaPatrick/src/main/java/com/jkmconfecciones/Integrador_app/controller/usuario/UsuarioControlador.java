...
import com.jkmconfecciones.Integrador_app.entidades.DetalleCotizacion;
...
public class UsuarioControlador {
    ...
    @GetMapping("/historial-cotizaciones")
    @ResponseBody
    public java.util.List<Map<String, Object>> obtenerHistorialCotizaciones() {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String correo = auth.getName();
        Usuario usuario = usuarioRepositorio.findByCorreo(correo)
                .orElseThrow(() -> new RuntimeException("Usuario no encontrado"));

        java.util.List<Cotizacion> cotizaciones = cotizacionRepositorio.findByUsuario(usuario);
        java.util.List<Map<String, Object>> resultado = new java.util.ArrayList<>();

        for (Cotizacion cot : cotizaciones) {
            Map<String, Object> map = new HashMap<>();
            map.put("id", cot.getId());
            map.put("fecha", cot.getFecha());
            map.put("estado", cot.getEstado());
            
            // Contar cantidad total de items (suma de cantidades)
            int totalItems = 0;
            if (cot.getDetalles() != null) {
                for (DetalleCotizacion detalle : cot.getDetalles()) {
                    totalItems += detalle.getCantidad() != null ? detalle.getCantidad() : 0;
                }
            }
            map.put("totalItems", totalItems);
            
            // Obtener la imagen del primer producto
            String imagenPrimer = "/images/placeholder.png";
            if (cot.getDetalles() != null && !cot.getDetalles().isEmpty()) {
                ProductoTalla pt = cot.getDetalles().get(0).getProductoTalla();
                if (pt != null && pt.getProducto() != null && pt.getProducto().getImagenUrl() != null) {
                    imagenPrimer = pt.getProducto().getImagenUrl();
                }
            }
            map.put("imagen", imagenPrimer);
